...
from custom_datasets import music_video_frames
...
def music_video_loader(path=""):
    def loader(transform):
        data = music_video_frames(folder=path, transform=transform, frame_batch=frame_batch)
        data_loader = DataLoader(data, shuffle=True, batch_size=batch_size,
                                 num_workers=4)
        return data_loader
    return loader
...
def train(generator, discriminator, init_step, loader, total_iter=600000):
    ...
    for i in pbar:
        ...
        try:
            ...
            dataset = iter(data_loader)
            real_image, label, noise_vector = next(dataset)
        # here label is the audio feat, real_image is the video frame corresponding to that audio feat
        #pdb.set_trace()
        iteration += 1
        ...
        if (i + 1) % 1000 == 0 or i==0:
            with torch.no_grad():
                images = g_running(gen_z, step=step, alpha=alpha).data.cpu() #torch.randn(5 * 10, input_code_size).to(device)
                #pdb.set_trace()
                images = images.view(b_size*frame_batch, 3, images.shape[-2], images.shape[-1])
                utils.save_image(
                    images,
                    str(log_folder)+"/sample/"+str(i + 1).zfill(6)+".png",
                    nrow=10,)
                    #normalize=True,
                    #range=(-1, 1))
        ...
        if (i+1) % 10000 == 0 or i==0:
            try:
                torch.save(g_running.state_dict(), log_folder+'/checkpoint/'+str(i + 1).zfill(6)+'_g.model')
                torch.save(discriminator.state_dict(), log_folder+'/checkpoint/'+str(i + 1).zfill(6)+'_d.model')
            except:
                pass

        if (i+1)%500 == 0:
            state_msg = (str(i + 1)+'; G: '+str(gen_loss_val/(500//n_critic))+'; D: '+str(disc_loss_val/500)+ \
            ' Grad: '+str(grad_loss_val/500)+'; Alpha: '+str(alpha))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default="/dataSRI/ARIJIT/music_video_gan/data/music_video_data/dance", help='path of specified dataset, should be a folder that has one or many sub image folders inside')
    ...
    parser.add_argument('--z_dim', type=int, default=256, help='the initial latent vector\'s dimension, can be smaller such as 64, if the dataset is not diverse')
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='how many batches of frames to train together at one iteration')
    parser.add_argument('--frame_batch', type=int, default=1, help='how many frames to predict together to learn continuity')
    ...
    generator = Generator(in_channel=args.channel, out_channel= frame_batch*3, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh).to(device)
    discriminator = Discriminator(feat_dim=args.channel, in_channels=frame_batch*3).to(device)
    g_running = Generator(in_channel=args.channel, out_channel= frame_batch*3, input_code_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh).to(device)
